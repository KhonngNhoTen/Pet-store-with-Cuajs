"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const RouteNormalization_1 = __importDefault(require("../RouteNormalization/RouteNormalization"));
class RouteLoader {
    constructor(options) {
        var _a, _b;
        this.normalization = (_a = options === null || options === void 0 ? void 0 : options.normalization) !== null && _a !== void 0 ? _a : RouteNormalization_1.default;
        this.routeHandlers = (_b = options === null || options === void 0 ? void 0 : options.routeHandlers) !== null && _b !== void 0 ? _b : [];
    }
    handler(routes, router) {
        return __awaiter(this, void 0, void 0, function* () {
            routes.forEach((route) => {
                route.registry(router);
            });
        });
    }
    load(routes, router) {
        return __awaiter(this, void 0, void 0, function* () {
            let data;
            for (let i = 0; i < this.routeHandlers.length; i++)
                data = yield this.routeHandlers[i].handler(routes, data);
            yield this.handler(routes, router);
            console.log("Done registry apis");
        });
    }
}
exports.default = RouteLoader;
